...
class WatchListMoviesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Watchlist Movies" , style: Theme.of(context).textTheme.displaySmall,),
        leading: _getBackFilledIconButton(context),
        leadingWidth: 80,
        automaticallyImplyLeading: false,
      ),
      body: BlocBuilder<WatchListMoviesCubit, WatchListMoviesState>(
          builder: (context , state){
            switch(state){
              case WatchListMoviesInitial() || WatchListMoviesLoading():{
                return Column(
                  children: [
                    Expanded(
                      child: Skeletonizer(
              ...
              case WatchListMoviesFailed():{
                return Center(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      DefaultFailedToLoadWidget(
                          errorMessage: "Failed to get WatchList movies :-(",
